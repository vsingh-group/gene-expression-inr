...
def merge_5d(mode, donor):
    df = pd.read_csv(f"data/abagendata/{mode}_{donor}.csv")
    annot = pd.read_csv(f"data/abagendata/annot_{donor}_4d.csv")
    ...
    merged_df.to_csv(f"data/abagendata/{mode}_{donor}_merged.csv", index=False)
