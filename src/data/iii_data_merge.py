...
def merge_5d(mode, donor, matter='83'):
    df = pd.read_csv(f"data/abagendata/train_{matter}_new/{mode}_{donor}.csv")
    annot = pd.read_csv(f"data/abagendata/train_{matter}_new/{matter}_annotation_{donor}_4d.csv")
    ...
    merged_df.to_csv(f"data/abagendata/train_{matter}_new/{mode}_{donor}_merged.csv", index=False)
