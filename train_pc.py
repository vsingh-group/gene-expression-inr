...
def get_brain_pc1(filepath='./data/pc1_merged.csv'):
    meta_df = pd.read_csv(filepath)
    vals = torch.tensor(meta_df[['value']].values, dtype=torch.float32)
    coords = torch.tensor(meta_df[['mni_x', 'mni_y', 'mni_z', 'classification', 'pc1']].values, dtype=torch.float32)
    return coords, vals
...
class BrainFitting(Dataset):
    def __init__(self, idx=0, normalize=True):
        super().__init__()
        self.coords, self.vals = get_brain_pc1()
...
def main():
    try:
        ...
        brain_siren = Siren(in_features=5, out_features=1, hidden_features=512, 
        ...
        optim = torch.optim.Adam(lr=1e-3, params=brain_siren.parameters())
        ...
        torch.save(brain_siren.state_dict(), f'./models_test/model_pc1.pth')
        ...
        with open(f'./models_test/max_min_values_pc1.csv', 'a') as file:
            writer = csv.writer(file)
            row = [str(value) for value in min_max_dict.values()]
            writer.writerow(row)
            
        logging.info(f"[Success]--pc1")

    except Exception as e:
        logging.error(f"[Error]--pc1--{e}")
